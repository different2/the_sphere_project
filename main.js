...
// Face-layout math + composite texture generation (shared with
// sphere-field.js - see face-texture.js's header).
import {
    createTangentBasis,
    getFaceCenters,
    getAngularRadiusForFaceCount,
    generateFullWrapTexture,
...
// ============================================================
// Face-layout math + composite texture generation
//
// Moved to face-texture.js (shared with sphere-field.js): spawned
// spheres now reuse the exact same per-face gnomonic-projection
// compositing as the main globe, instead of only ever being able to
// show one full-sphere-wrap image. See face-texture.js's header.
// ============================================================
...
async function loadLaughingMan() {
    ...
    try {
        ...
        const angularRadius =
            getAngularRadiusForFaceCount(6, faceSizeMultiplier);
...
async function bakeFramesToTextures(sourceFrames, faceCountForBake, onProgress) {
    ...
    const angularRadius =
        getAngularRadiusForFaceCount(faceCountForBake, faceSizeMultiplier);
...
function updateFaceComposite() {
    ...
    const angularRadiusDeg =
        getAngularRadiusForFaceCount(faceCount, faceSizeMultiplier);
    ...
    (input) => {
        ...
        input.addEventListener(
            ...
            async (e) => {
                ...
                // Field mode with a selected sphere: Faces drives
                // that sphere's own per-face composite, not the main
                // globe underneath it.
                const target = fieldTarget();
    ...
    for (let i = 0; i < MAX_FACE_SLOTS; i++) {
        ...
        slotFileInput.addEventListener(
            ...
            (e) => {
                ...
                // ------------------------------------------------
                // Field mode with a selected sphere: route this
                // upload to THAT sphere's own face slot i, so
                // multiple images land on different sides of it
                // (see regenerateFieldSphereTexture in
                // sphere-field.js) instead of each upload
...
function renderFaceSlotThumbnails() {
    ...
    // Field mode with a selected sphere: thumbnails reflect THAT
    // sphere's own faces, not the main globe's.
    const target = fieldTarget();

    const effectiveFaceCount =
        target ? (target.faceCount || 1) : faceCount;

    const shouldShow =
        (target ? true : activeCustomKind === 'image') &&
    ...
    faceSlotButtons.forEach(
        (slot, i) => {
            ...
            if (target) {
                ...
                // Field spheres only ever hold plain <img>/<canvas>
                // sources (see faceImages in sphere-field.js) - no
                // GIF-thumbUrl case to handle here.
                const faceImg =
                    target.faceImages[i] ||
                    target.faceImages[0];
                ...
                const previewSrc =
                    faceImg ?
                        (faceImg.src ||
                            (faceImg.toDataURL && faceImg.toDataURL())) :
                        null;
...
onSelectionChanged((sphere) => {
    ...
    // The Faces radio is one shared UI element, but each sphere (and
    // the main globe) has its own independent faceCount. Without
    // this, the radio keeps showing whatever was last clicked - e.g.
    // still "3" after switching to (or spawning) a sphere that's
    // actually at the default of 1 - while per-face uploads
    // correctly follow the SPHERE's real faceCount underneath. That
    // mismatch is exactly what made per-face uploads look like they
    // "sometimes work, sometimes don't": whether it worked depended
    // on whether the radio's stale value happened to match reality.
    const effectiveFaceCount =
        sphere ? (sphere.faceCount || 1) : faceCount;
