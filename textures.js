...
//
// Renders arbitrary user-supplied text onto a canvas, auto-sizing
// the font so it fits. Returns the CANVAS itself (not a data URL)
// so callers can either use it directly as a full-globe wrap, or
// feed it into generateGlobeTexture() to place it as 1-6 logo-style
// faces on the sphere - same as an uploaded image.
//
// NOTE: this canvas is NOT pre-flipped. Whichever code samples it
// onto the sphere is responsible for orientation (see main.js -
// generateFullWrapTexture() applies the flip that a full equirect
// wrap needs; the multi-face logo placement path needs no flip).
// ============================================================
function createTextTexture(text) {
    ...
    // Main message, auto-fit to width
    const message = (text && text.trim().length > 0) ? text.trim() : 'HELLO WORLD!';
    ...
    do {
        ...
        if (width <= maxWidth || fontSize <= 12) break;
        fontSize -= 4;
    } while (true);
    ...
    ctx.fillText('Custom Globe Texture', canvas.width / 2, canvas.height / 2 + fontSize * 0.75 + 20);
