const fragmentShader = /*glsl*/`
    ...
    // More efficient nearest dot finding using analytical approach
    float findNearestDot(vec3 rP, float dots, out vec3 gP) {
        // For Fibonacci sphere, we can use a more direct approach
        // Convert Cartesian to spherical coordinates
        float phi_sphere = asin(clamp(rP.y, -1.0, 1.0));
        float theta_sphere = atan(rP.z, rP.x);
        
        // Normalize phi to [0, 1] range
        float phi_norm = (phi_sphere + PI/2.0) / PI;
        
        // Estimate the closest index based on y-coordinate
        float estimated_idx = (1.0 - rP.y) * (dots - 1.0) * 0.5;
        
        // Search in a small neighborhood around the estimated index
        float minDist = 1000.0;
        gP = vec3(0.0);
        
        // Check nearby indices (usually within ±10 is sufficient)
        for (int offset = -10; offset <= 10; offset++) {
            float idx = estimated_idx + float(offset);
            if (idx < 0.0 || idx >= dots) continue;
            
            vec3 dotPos = fibonacciSphere(idx, dots);
    ...
    void main() {
        ...
        if (l <= r * r) {
            for (int side = 0; side <= 1; side++) {
                ...
                // Find the nearest dot using the efficient method
                vec3 gP;
                float dis = findNearestDot(rP, dots, gP);
